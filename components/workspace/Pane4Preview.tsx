...
export function Pane4Preview({ selectedClient, text, targetMonth, generationCount, pdfBase64, onTextChange, onSave }: Props) {
  const now = new Date();
...
【採点基準】
A：社長目線（/100）
- 構成のわかりやすさ（/25）：1分で状況が把握できる構成になっているか
- 自社固有性（/25）：この会社固有の数字・文脈・前月比較を使っているか。業種・規模を踏まえた分析か。一般論で説明していないか
- 情報の完全性（/25）：経営者が今月知るべき重要数字（キャッシュ・損益・前期比）が揃っているか。経営者側で確認・判断が必要なことがあれば明示されているか
- 安心感・信頼感（/25）：悪い数字も誠実に伝えているか。ポジティブな情報で問題を薄めたり後回しにしていないか

B：税理士品質（/100）
- 数字の透明性（/25）：赤字・前期比マイナス等のネガティブ数値が明記されているか。定性的な表現で数字の説明を代替していないか
- 指摘の鋭さ（/25）：異常値・構造的問題を的確に捉えているか
- リスク言及（/25）：資金・収益・特定顧客依存等のリスクを明示しているか
- 将来への提言（/25）：提言はその月の数字が示す因果関係に基づいているか。可能な範囲で定量的な見通しを含んでいるか

【前提条件】
- 読む媒体：Chatwork
- 書き手：税理士（クライアント企業の経営判断・営業活動への指示は役割外）
- 社長のタイプ：返信しない
- 目標：社長が読んで安心し、今月の状況と次に必要なことがわかる報告

【採点時の絶対ルール】
1. 将来の税務日付は正当：消費税・法人税の中間納付・確定申告等の法定スケジュールは報告時点より未来の日付であることが通常であり、日付誤りとして指摘してはならない
2. アクション宣言の主語は税理士のみ：「来月は〇〇に注力します」「△△を進めましょう」など、クライアント企業の営業活動・採用・投資等の行動を指示・宣言する文言は税理士の役割外のため、改善点として指摘してはならない。税理士自身の行動（確認・試算・申告・提出・再計算等）の宣言のみを高評価とする
3. 副産物の文脈誤読禁止：売上減少・費用増加等の原因によって発生した税務上の数字（例：売上減少による消費税還付見込み）を、独立したポジティブ機会として提案（例：「還付金を投資へ活用」）することは文脈誤読であり、改善点として提示してはならない
